...
const PlacementHighlights = () => {
  ...
  useEffect(() => {
    ...
    const updateValues = (setter, limit) =>
      setInterval(() => setter((prev) => (prev < limit ? prev + 1 : limit)), 30);
  ...
  const doughnutData = {
    labels: ["IT", "Core Engineering", "Finance", "Consulting", "Other Sectors"],
    datasets: [
      {
        data: [40, 25, 15, 10, 10],
        backgroundColor: ["#2563eb", "#10b981", "#facc15", "#f97316", "#db2777"],
        hoverOffset: 6,
      },
    ],
  };
  ...
  const handleSocialMediaShare = (platform) => {
    const url = encodeURIComponent("https://www.your-website-link.com"); // Replace with your website URL
  
    if (platform === "linkedin") {
      // Attempt to open LinkedIn app or fall back to browser
      window.open(`linkedin://shareArticle?mini=true&url=${url}`, "_blank") || 
      window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${url}`, "_blank");
    } 
    else if (platform === "twitter") {
      // Attempt to open Twitter app or fall back to browser
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText("https://www.your-website-link.com") // Replace with your link
      .then(() => toast.success("Link copied to clipboard!"))
      .catch(() => toast.error("Failed to copy link"));
  };

  const handleDownloadPDF = () => {
    const pdfUrl = "/path-to-your-pdf/placement-report.pdf"; // Replace with the actual PDF path
...
return (
    <section className="bg-white py-16 px-6 lg:px-20 relative">
        <div className="max-w-6xl mx-auto text-center">
            ...
            <p className="text-gray-600 text-lg mb-12 text-base sm:text-sm lg:text-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="p-6 bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg shadow-lg h-[50vh] md:h-[85vh]">
                    <h3 className="text-2xl font-bold mb-4 text-gray-800">Placement Trends</h3>
                    <Bar data={barData} options={{ responsive: true, maintainAspectRatio: false }} />
                </div>
                <div className="p-6 bg-gradient-to-br rounded-lg shadow-lg h-[50vh] md:h-[85vh]">
                    <h3 className="text-xl font-bold text-gray-800">Placement by Sector</h3>
